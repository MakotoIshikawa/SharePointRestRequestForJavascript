function $_global_clientforms() {
	...
	SPClientForms.ClientForm = function (r) {
		...
		this.SubmitClientForm = function () {
			if (k || g == SPClientTemplates.ClientControlMode.DisplayForm)
				return false;
			var n = "";
...
function IsMinimized() {
	var b = new URI(document.URL),
		a = b.getQueryParameter("Minimized");
	return Boolean(a)
}
...
function SPField_FormDisplay_Core(d, e) {
	...
	if (a == null || a.fieldSchema == null || a.fieldValue == null)
		return "";
	var b = a.fieldValue;
...
function SPFieldText_Edit(h) {
	if (h == null)
		return "";
	var a = SPClientTemplates.Utility.GetFormContextForCurrentField(h);
	if (a == null || a.fieldSchema == null)
		return "";
	var b, g = a.fieldValue != null ? a.fieldValue : "",
	...
	var f = a.fieldSchema.IMEMode == "" ? "" : 'style="ime-mode : ' + STSHtmlEncode(a.fieldSchema.IMEMode) + '"';
	if (!(window.OffSwitch == null || OffSwitch.IsActive("1521D456-1FBD-4B49-8832-1A945A77FDF4")))
		if (Boolean(IsMinimized())) f = 'style="width: 265px;"';
	var c = '<span dir="' + STSHtmlEncode(a.fieldSchema.Direction) + '">';
	c += '<input type="text" value="' + STSHtmlEncode(g) + '" maxlength="' + STSHtmlEncode(a.fieldSchema.MaxLength) + '" ';
...
function SPFieldNote_Edit(i) {
	...
	function J() {
		var b = '<span dir="' + o + '">';
		b += '<textarea rows="' + STSHtmlEncode(a.fieldSchema.NumberOfLines) + '" cols="20"';
		if (!(window.OffSwitch == null || OffSwitch.IsActive("1521D456-1FBD-4B49-8832-1A945A77FDF4")))
			if (Boolean(IsMinimized())) b += ' style="width: 265px;"';
		b += ' id="';
...
function SPFieldFile_Display(k) {
	...
	if (c == null || c.fieldSchema == null || c.fieldValue == null || c.itemAttributes == null)
		return "";
	var b = c.fieldValue,
	...
	if (i == SPClientTemplates.FileSystemObjectType.File) {
		var d = b.FileUrl,
			h = b.BaseName,
			g = b.File_x0020_Type_progid,
			e = b.serverurl_progid,
			f = c.listAttributes.DefaultItemOpen;
		h = h == null ? "" : STSHtmlEncode(h);
		d = d == null ? "" : STSHtmlEncode(d);
		if (Flighting.VariantConfiguration.IsExpFeatureClientEnabled(54)) d = d.replace(/#/g, "%23");
		g = g == null ? "" : STSHtmlEncode(g);
		f = f == null ? "" : STSHtmlEncode(f);
		e = e == null ? "" : STSHtmlEncode(e);
		var a = "";
		a += '<a rel="sp_DialogLinkNavigate" href="' + d + '" ';
		a += "onmousedown=\"return VerifyHref(this, event, '" + f;
		a += "', '" + g + "', '" + e;
...
function SPFieldFile_Edit(k) {
	...
	var c, d = a.fieldValue != null ? a.fieldValue : "",
		f = a.fieldName + "_" + a.fieldSchema.Id + "_$onetidIOFile",
		e = "",
	...
	a.registerGetValueCallback(a.fieldName, function () {
		return c == null ? "" : SPClientTemplates.Utility.Trim(c.value) + e
	});
	a.updateControlValue(a.fieldName, d);
	var b = "";
	if (j) {
		...
		b += '<input type="text" value="' + STSHtmlEncode(o) + '" maxlength="' + (400 - e.length).toString() + '" id="' + STSHtmlEncode(f) + '" ';
		b += 'title="' + STSHtmlEncode(a.fieldSchema.Title);
		if (a.fieldSchema.Required) b += " " + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
		b += '" class="ms-input" size="35" />';
		b += '<span class="ms-fileField-fileExt">' + e + "</span></span><br /></span>";
		return b
	} else if (i) {
		b = '<span dir="' + STSHtmlEncode(a.fieldSchema.Direction) + '">';
		b += '<input type="text" value="' + STSHtmlEncode(d) + '" maxlength="' + (400).toString() + '" id="' + STSHtmlEncode(f) + '" ';
		b += 'title="' + STSHtmlEncode(a.fieldSchema.Title);
		if (a.fieldSchema.Required) b += " " + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
	...
	function m() {
		c != null && a.updateControlValue(a.fieldName, SPClientTemplates.Utility.Trim(c.value) + e)
	}

	function l(d) {
		if (j || i) {
			if (c == null) c = document.getElementById(f);
			if (c != null) {
				var a = c.parentNode;
				if (a != null) {
					var b = f + "_parentSpan";
...
function SPFieldChoice_Dropdown_Edit(v) {
	...
	if (d)
		return '<span dir="' + STSHtmlEncode(a.fieldSchema.Direction) + '">' + x() + "</span>";
...
function SPFieldChoice_Radio_Edit(q) {
	...
	function o() {
		if (c != null && c.checked && b != null)
			return SPClientTemplates.Utility.Trim(b.value);
		else
			for (var d = 0; d < e.length; d++) {
				var a = e[d];
				if (a != null && a.checked)
...
function SPFieldDateTime_Edit(t) {
	...
	function x() {
		...
		var f = '<label for="' + l + '" class="ms-accessible">';
		f += STSHtmlEncode(StBuildParam(Strings.STS.L_DateTimeFieldDateHoursLabel, a.fieldTitle)) + "</label>";
		f += '<select id="' + l + '" ' + k + ">";
		for (var n = b.HoursOptions, e = 0; e < n.length; e++) f += m(String(e), n[e], e == Number(z));
		f += "</select>";
		var p = ["00", "05", "10", "15", "20", "25", "30", "35", "40", "45", "50", "55"],
			d = '<label for="' + j + '" class="ms-accessible">';
		d += STSHtmlEncode(StBuildParam(Strings.STS.L_DateTimeFieldDateMinutesLabel, a.fieldTitle)) + "</label>";
		d += '<select id="' + j + '" ' + k + ">";
...
function SPFieldUrl_Display(g) {
	...
	if (b.fieldSchema.DisplayFormat == SPClientTemplates.UrlFormatType.Hyperlink) {
		if (a.URL != "" && a.URL != "http://")
			return d + '<a href="' + STSHtmlEncode(a.URL) + '" target="_blank">' + STSHtmlEncode(a.Description) + "</a>" + f
	} else if (b.fieldSchema.DisplayFormat == SPClientTemplates.UrlFormatType.Image)
		if (a.URL != "" && a.URL != "http://")
			return d + '<img src="' + STSHtmlEncode(a.URL) + '" alt="' + STSHtmlEncode(a.Description) + '"/>' + f;
	return ""
}

function SPFieldUrl_Edit(p) {
	if (p == null)
		return "";
	var a = SPClientTemplates.Utility.GetFormContextForCurrentField(p);
	if (a == null || a.fieldSchema == null)
		return "";
	var o = a.fieldValue != null ? a.fieldValue : "",
		n = SPClientTemplates.Utility.ParseURLValue(o),
		t = n.URL,
		s = n.Description,
		c, d, h = a.fieldName + "_" + a.fieldSchema.Id,
		f = h + "_$UrlFieldUrl",
		q = STSHtmlEncode(h + "_$UrlControlId"),
		i = h + "_$UrlFieldDescription",
		e = a.fieldName,
		m = a.fieldSchema.IMEMode == "" ? "" : 'style="ime-mode : ' + STSHtmlEncode(a.fieldSchema.IMEMode) + '" ';
	if (!(window.OffSwitch == null || OffSwitch.IsActive("1521D456-1FBD-4B49-8832-1A945A77FDF4")))
		if (Boolean(IsMinimized())) m = 'style="width: 265px;"';
	var g = new SPClientForms.ClientValidation.ValidatorSet;
	a.fieldSchema.Required && g.RegisterValidator(new SPClientForms.ClientValidation.RequiredUrlValidator);
	g.RegisterValidator(new SPClientForms.ClientValidation.MaxLengthUrlValidator(255));
	a.registerClientValidator(a.fieldName, g);
	a.registerInitCallback(a.fieldName, r);
	a.registerFocusCallback(a.fieldName, function () {
		c != null && c.focus()
	});
	a.registerValidationErrorCallback(a.fieldName, function (a) {
		SPFormControl_AppendValidationErrorMessage(f, a)
	});
	a.registerGetValueCallback(a.fieldName, k);
	a.updateControlValue(a.fieldName, o);
	var j = STSHtmlEncode(f),
		b = '<span dir="' + STSHtmlEncode(a.fieldSchema.Direction) + '"><span class="ms-formdescription">';
	b += STSHtmlEncode(Strings.STS.L_UrlFieldTypeText) + '&#160;(<a id="' + q + '" aria-describedby="' + e + '"';
	b += "href=\"javascript:TestURL('" + j + '\')" target="_self">';
	b += STSHtmlEncode(Strings.STS.L_UrlFieldClickText) + "</a>)<br /></span>";
	b += '<input dir="ltr" type="text" value="' + STSHtmlEncode(t) + '" id="' + j + '" aria-describedby="' + e + '" aria-label="';
	b += STSHtmlEncode(Strings.STS.L_UrlFieldTypeText);
	if (Boolean(Strings.STS.L_RequiredField_Text))
		if (a.fieldSchema.Required) b += " " + STSHtmlEncode(Strings.STS.L_RequiredField_Text);
	b += '" class="ms-long" ' + m + "/><br />";
	b += '<span class="ms-formdescription">' + STSHtmlEncode(Strings.STS.L_UrlFieldTypeDescription) + "&#160;<br /></span>";
	b += '<input type="text" maxlength="';
	b += String(255);
	b += '" id="' + STSHtmlEncode(i) + '" title="';
	b += STSHtmlEncode(Strings.STS.L_UrlFieldDescriptionTitle) + '" aria-describedby="' + e + '"';
	if (!(window.OffSwitch == null || OffSwitch.IsActive("1521D456-1FBD-4B49-8832-1A945A77FDF4")))
		if (Boolean(IsMinimized())) b += ' style="width: 265px;"';
	b += ' value="' + STSHtmlEncode(s) + '" class="ms-long" /><br /></span>';
	return b;
	...
	function r() {
		...
		if (c != null) {
			AddEvtHandler(c, "onchange", l);
			AddEvtHandler(c, "onfocus", function () {
				c.select()
			})
		}
		d != null && AddEvtHandler(d, "onchange", l)
	}

	function l() {
		a.updateControlValue(a.fieldName, k())
	}
...
function SPClientPeoplePickerCSRTemplate(v) {
	if (v == null)
		return "";
	var a = SPClientTemplates.Utility.GetFormContextForCurrentField(v);
	if (a == null || a.fieldSchema == null)
	...
	var d = a.fieldSchema.ServerContainerId != null ? a.fieldSchema.ServerContainerId : a.fieldName + "_" + a.fieldSchema.Id + "_$ClientPeoplePicker",
		...
		f = d + "_HiddenInput",
		y = a.fieldSchema.ServerInputName != null ? STSHtmlEncode(a.fieldSchema.ServerInputName) : f;
	a.fieldSchema.TopLevelElementId = d;
	a.fieldSchema.EditorElementId = m;
	a.fieldSchema.AutoFillElementId = r;
	a.fieldSchema.NotificationElementId = o;
	a.fieldSchema.ResolvedListElementId = s;
	a.fieldSchema.InitialHelpTextElementId = p;
	a.fieldSchema.WaitImageId = t;
	...
	if (a.fieldSchema.Required && typeof a.registerClientValidator == "function") {
		var u = new SPClientForms.ClientValidation.ValidatorSet;
		u.RegisterValidator(new SPClientForms.ClientValidation.RequiredValidator);
		a.registerClientValidator(a.fieldName, u)
	}
	var B = a.fieldSchema.InPlaceEditMode ? "ms-grid" : "sp-peoplepicker-topLevel",
		q = a.fieldSchema.InPlaceEditMode ? 'style="padding-left:3px;padding-top:1px;cursor:text;height:100%;border-width:1px;position:relative;min-width:' + (a.fieldSchema.Width - 8).toString() + 'px"' : "";
	if (!(window.OffSwitch == null || OffSwitch.IsActive("1521D456-1FBD-4B49-8832-1A945A77FDF4")))
		if (Boolean(IsMinimized())) q = 'style="width: 250px;"';
	var z = a.fieldSchema.InPlaceEditMode ? "" : e,
		x = a.fieldSchema.InPlaceEditMode ? 'style="margin:0px"' : "",
		b = '<div dir="' + STSHtmlEncode(a.fieldSchema.Direction) + '">';
	b += '<div class="' + B + '" id="' + STSHtmlEncode(d) + '" title="' + l + '"' + q + "/>";
	b += '<input id="' + STSHtmlEncode(f) + '" name="' + STSHtmlEncode(y) + '" type="hidden" />';
	b += '<div id="' + STSHtmlEncode(r) + '" class="sp-peoplepicker-autoFillContainer"></div>';
	b += '<span id="' + STSHtmlEncode(p) + '" class="sp-peoplepicker-initialHelpText ';
	b += 'ms-helperText">' + STSHtmlEncode(z) + "</span>";
	b += '<img class="sp-peoplepicker-waitImg" id="' + STSHtmlEncode(t) + '" alt="';
	b += STSHtmlEncode(Strings.STS.L_SPClientPeoplePickerWaitImgAlt) + '" src=';
	b += '"/_layouts/15/images/gears_anv4.gif?rev=44"/>';
	b += '<span id="' + STSHtmlEncode(s) + '" class="sp-peoplepicker-resolveList"></span>';
	b += '<span id="' + STSHtmlEncode(o) + '" aria-live="polite" style="visibility:hidden;position:absolute;width:0px;display:inline-block;"></span>';
	b += '<input type="text" class="sp-peoplepicker-editorInput" size="1" autocomplete="off" value="" aria-required="true" ';
	b += 'id="' + STSHtmlEncode(m) + '" title="' + l + '"' + x + ' autocorrect="off" autocapitalize="off" /></div>';
	b += "</div></div>";
...
function SPFieldLookupMulti_Edit(y) {
	...
	function u() {
		if (g == null || typeof GipSplit == "undefined")
			return "";
...
function SPFieldAttachments_Default(o) {
	...
	if (b == null || b.fieldSchema == null || b.fieldValue == null)
		return "";
	var e, h = "attachmentsToBeRemovedFromServer";
	...
	if (b.fieldValue != "")
		for (var n = b.fieldValue.Attachments, s = STSHtmlEncode(b.fieldValue.UrlPrefix), k = STSHtmlEncode(Strings.STS.L_DeleteVersion_Text), f = 0; f < n.length; f++) {
			var c = n[f],
				r = STSHtmlEncode(c.FileName),
				m = Flighting.VariantConfiguration.IsExpFeatureClientEnabled(54) ? STSHtmlEncodeEx(c.FileName) : STSHtmlEncode(c.FileName),
				j = Flighting.VariantConfiguration.IsExpFeatureClientEnabled(54) ? c.RedirectUrl === null ? "" : encodeScriptQuote(c.RedirectUrl) : STSHtmlEncode(c.RedirectUrl),
				i = Flighting.VariantConfiguration.IsExpFeatureClientEnabled(54) ? STSHtmlEncodeEx(c.AttachmentId) : STSHtmlEncode(c.AttachmentId),
				d = STSHtmlEncode(c.FileTypeProgId);
			if (d == "") d = "null";
			var l = b.listAttributes.DefaultItemOpen ? "1" : "0";
			a += '<tr id="';
			a += i;
			a += '"><td class="ms-vb"><span dir="ltr">';
			a += "<a onmousedown=\"return VerifyHref(this, event, '";
			a += l;
			a += "', '";
			a += d;
			a += "', '";
			...
			if (b.controlMode == SPClientTemplates.ClientControlMode.EditForm) {
				...
				a += i;
				a += "', 1); return false;\"";
				a += ' aria-label="' + STSHtmlEncode(String.format(Strings.STS.L_DeleteAttachment_Text, m));
				a += '">&nbsp;';
				a += k;
				a += "</a></td>"
			}
	...
	function q() {
		if (typeof FileUploadLocalFileCount != "undefined" && FileUploadLocalFileCount > 0)
			return true;
		if (e == null) e = document.getElementsByName(h)[0];
		return e != null && e.value != "" ? true : false
	}
}
